...
public class PololuConnecter : MonoBehaviour
{
    ...
    public bool TrySetTarget(Byte channel, UInt16 target, string serial = null)
    {
        if(serial == null)
        {
            try
        ...
        else
        {
            try
            {
                using (Usc device = ConnectToDeviceWithSerial(serial))  // Find a device and temporarily connect.
                {
                    device.setTarget(channel, target);
    ...
    public bool ConnectResult(string serial)
    {
        try
        {
            using (Usc device = ConnectToDeviceWithSerial(serial))  // Find a device and temporarily connect.
    ...
    Usc ConnectToDevice()
    {
        ...
        Debug.Log(connectedDevices[0].serialNumber);
    ...
    Usc ConnectToDeviceWithSerial(string Serial)
    {
        // Get a list of all connected devices of this type.
        List<DeviceListItem> connectedDevices = Usc.getConnectedDevices();
        //Debug.Log(connectedDevices[0].serialNumber);
